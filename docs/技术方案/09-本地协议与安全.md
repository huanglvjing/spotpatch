...
Astro 补充（2026-09-05）：源码扩展名白名单新增 `.astro`，但仍须会话 registry、realpath root、大小预算与 token/Origin 校验。astro-host 目标使用现有 annotation authorizer 重新读取、授权和裁剪源码，不信任浏览器提交的片段。中间件同 origin 挂载，不另起端口；Astro base 不改变 `/__spotpatch` 协议根路径。build/preview/sync 不挂载中间件。详见 [Astro 安全设计](./Astro适配/00-方案与验收.md)。
