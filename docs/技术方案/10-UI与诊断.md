...
source-range: "规格书 §2.4 第 3 条、§18、§18.1–§18.3、§22；v1.1 Agent 工作台与变更审阅"
参考文献/依赖:
...
- AI 未启用或 capability 未通过时，`Preview prompt` 保持主操作；AI 就绪时，`Run AI` 成为问题输入态的主操作，`Preview prompt` 作为明确可用的本地回退，不能被隐藏。
...
- 运行按钮旁必须显示当前模型；`Mod+Enter` 等价于一次明确运行，不得在输入、选择或模型切换时自动发起请求。
