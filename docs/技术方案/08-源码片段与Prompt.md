...
- 对相互独立的只读证据应在同一轮并行请求；review/auto 最终写入后只运行已登记的相关 check，宿主会复用同一变更版本的真实结果并补跑缺失的 required checks。trusted-auto 优先使用上下文中的精确源码路径，不下发 `run_check`，成功写入后直接结束。没有必要变更时必须明确返回无变更结果。
