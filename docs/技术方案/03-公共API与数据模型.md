...
export interface SpotPatchOptions {
  ...
  /**
   * 默认 false。为 true 时启用页面可选的可信快速模式能力；
...
`trustedFastMode: true` 是低配置入口，不是绕过验证的开关。Vite/Next 适配器必须在 Git 项目根内发现普通的根 `tsconfig.json`、项目声明的 `typescript` 依赖和可解析的本地 TypeScript CLI 后，生成 `trusted-auto` 能力及唯一的 required TypeScript check；命令以固定 argv 和 `shell: false` 执行，不经过包管理器脚本。无法安全发现时必须给出明确配置错误，初始化器则只生成审阅模式配置。显式 `ai.execution` 始终优先：已有 required checks 可以与该简写复用，`applyMode: "auto"` 与简写冲突时必须拒绝，不能静默改变用户策略。页面仍默认选择 `review`，只有用户主动切换并完成当前会话授权后才运行 `trusted-auto`。
