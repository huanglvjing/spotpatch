...
source-range: "规格书 §2.1–§2.2、§3–§5（§5.1 除外）、§5.2"
参考文献/依赖:
  - "01-product-boundary"
...
本架构以 v1 产品定义和支持边界为约束 (见 doc-id:01-product-boundary)。
...
图中的 source marker 与 fileId 由 Vite 插件规范定义 (见 doc-id:04-vite-plugin)，local protocol 由本地协议与安全规范定义 (见 doc-id:09-local-protocol-security)。
