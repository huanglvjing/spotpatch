...
`trustedFastMode: true` 是低配置的显式直改能力。Vite/Next 适配器必须在 Git 项目根内发现普通的根 `tsconfig.json`、项目声明的 `typescript` 依赖和可解析的本地 TypeScript CLI 后，生成 `trusted-auto` 能力及唯一的 required TypeScript check；命令以固定 argv 和 `shell: false` 执行，不经过包管理器脚本。该检查只保护页面切回的 review 流程，trusted-auto 不向模型下发 `run_check` 且不执行最终项目检查。无法安全发现时必须给出明确配置错误；Vite 用户应保持 `spotPatch()` 的默认审阅配置，Next 初始化器则只生成审阅模式配置。显式 `ai.execution` 始终优先；`applyMode: "auto"` 与简写冲突时必须拒绝，不能静默改变用户策略。页面仍默认选择 `review`，只有用户主动切换、阅读跳过检查的后果并完成当前会话授权后才运行 `trusted-auto`。

AI 配置解析必须满足：URL、模型和运行时 Key 均存在；provider 和 model profile ID 非空且唯一；`defaultProvider` 与每个 `defaultModel` 都引用已登记 ID；`apiKeyEnv` 是不以 `VITE_` 开头的大写环境变量名；`authentication` 只接受 `bearer | x-api-key`；`baseURL`、协议和凭据规则通过 provider 校验；check ID 只含安全标识字符，命令非空，超时和 limits 为有限正整数。`applyMode: "auto" | "trusted-auto"` 至少配置一个 required check；auto 执行它，trusted-auto 仅保留它供 review 回退使用。`trusted-auto` 还要求每次创建 Job 都携带当前浏览器会话的显式可信极速模式同意；缺少必要值或存在部分约定式配置时，开发服务器启动失败并只报告缺少的变量名，不回显值，也不得静默降级为似乎可运行的 AI。执行语义见 Agent 规范 (见 doc-id:16-ai-agent-execution)，环境变量名、provider 与凭据语义见模型提供商规范 (见 doc-id:17-model-provider-credentials)。
