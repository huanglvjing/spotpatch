...
source-range: "规格书 §2.1–§2.2、§3–§5（§5.1 除外）、§5.2；v1.1 AI Agent 架构扩展；v1.2 产品化 Runtime 模块边界；v1.3 Runtime/React Adapter 浏览器包拆分；v1.4 CI 启动与 npm 发布基线；v1.4.1 跨平台文本基线；v1.5 编辑器适配边界；v1.6 编辑器工作区路由；v1.7 本地工作区健康与隔离快照边界；Next.js 多适配器公共预览架构"
参考文献/依赖:
  - "01-product-boundary"
