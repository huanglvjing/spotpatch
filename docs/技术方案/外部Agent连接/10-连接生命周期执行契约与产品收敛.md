...
下表记录 ADR-037 编码前的工作区基线，用于保留决策来源，不代表当前实现状态。当前状态以 (见 doc-id:external-agent-00-index) 的状态摘要和本文 P1–P4 Gate 为准。
...
| 兼容性锁死单个 Codex 字符串版本 | 已改为最低稳定版、逐 executable generated Schema 与运行时 preflight | 保持无未来版本上限；固定安全子集缺一项即 fail closed，不为表面兼容跳过校验 |
