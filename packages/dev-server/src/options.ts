...
import type { SourceFilterEntry } from "@spotpatch/compiler";
import { z } from "zod";
...
export type FilterEntry = SourceFilterEntry;
...
export function resolveOptions(
  ...
  if (
    resolved.shortcut.trim().length === 0 ||
    resolved.shortcut.length > 128 ||
    resolved.shortcut.includes("\0")
  ) {
