...
source-range: "规格书 §2.1–§2.2、§3–§5（§5.1 除外）、§5.2；v1.1 AI Agent 架构扩展；v1.2 产品化 Runtime 模块边界；v1.3 Runtime/React Adapter 浏览器包拆分；v1.4 CI 启动与 npm 发布基线；v1.4.1 跨平台文本基线"
参考文献/依赖:
  - "01-product-boundary"
...
- 不引入通用命令执行封装；打开编辑器、受控 Git 子命令和预配置检查分别经过窄接口 adapter，所有子进程都使用固定 `command + args` 且 `shell: false`。
