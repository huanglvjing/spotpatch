...
source-range: "规格书 §2.4 第 3 条、§18、§18.1–§18.3、§22；v1.1 Agent 工作台与变更审阅；v1.2 多目标工作台；v1.3 品牌视觉、双语与逐目标编辑；v1.4 低密度 Agent 布局与入口层级；v1.5 源码快捷入口与开源传播；v1.6 深色原生选择控件；v1.7 工作区健康、显式同意与渐进增强选择器；v1.8 工具轮次活动与精确错误诊断；v1.9 直接运行、可选能力诊断与严格 CSP nonce 继承；v2.0 可拖拽浮动工作台与灵动岛入口目标"
参考文献/依赖:
...
- 灵动岛入口默认位于右下角安全区域；用户可以拖拽灵动岛或展开工作台的标题栏改变位置。位置、拖拽、吸附、会话恢复和动效的唯一详细规范见 (见 doc-id:21-floating-workbench-island)。该专题的 Runtime 实现与单元校验已完成，真实 Chromium 视觉验证仍是发布前 Gate。
...
- 动画只允许由集中 motion token 控制的颜色、边框、轻微位移和浮动表面展开反馈；具体时长与曲线在完整 playground 验证后收敛，不能散落到组件。`prefers-reduced-motion` 下移除非必要过渡。
