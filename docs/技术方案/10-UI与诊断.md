...
- 当前 Trigger、Planner 与 Execution Island 已收敛为同一个 fixed Shell 内的互斥 Scene；“Pill → Capturing → Planner → Handoff → Running → Result”只消费真实状态，并由独立 GSAP Motion bundle 编排连续 Morph、方向光和有界粒子。代码与自动化 Gate 已完成，真实 Chromium 视觉/性能验收仍待补；详细边界见 (见 doc-id:22-persistent-shell-motion-system)。
