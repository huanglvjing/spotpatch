...
4. 在主动 adapter 已 ready 时立即收到任务：Claude attached 收到交接引用并按需读取已授权上下文；Codex managed 收到服务端构建的有界工程契约并只写独立临时快照；Inbox 本身不承诺自动注入当前对话；
5. Generic/attached 使用宿主自己的读取、编辑、审批和检查能力；Codex managed 由 SpotPatch 审计 diff、运行显式 required checks，并只在 Gate 全过后回写；
6. 在 SpotPatch 中看到连接、投递、厂商执行和 managed pipeline 各自有证据的状态，而不是模糊的“已发送成功”。
...
便捷体验来自一次本地授权和常驻 Node 生命周期所有者，而不是用一次性模型 tool call 模拟订阅。Claude attached 的 owner 是宿主 Channel Connector；Codex managed 的 owner 是随 `pnpm dev` 启动的 Supervisor：
...
sequenceDiagram
  ...
  participant U as 用户
  A->>C: 一次连接 / 启用 Channel 或授予 managed profile
  C->>S: 常驻有界等待（无模型 turn）
  U->>S: 选择 + 说明 + 发送
  S-->>C: 当前授权快照
...
不能从通用 MCP 推断“模型已阅读”“开始编辑”“检查通过”或“改动完成”。Claude Channel 的 transport write 也不是模型 ACK。厂商结构化事件只能更新 execution 证据；Codex `turn/completed` 仍不等于代码正确、checks 通过或已回写。managed `completed` 只来自 apply 后一致性证明。
...
| FR-14 | attached 权限由宿主显式授予；managed 首次固定 profile 由 dev 终端 challenge 授予。Browser API 禁止提交 executable、args、cwd、thread、model、sandbox、checks 或 approval |
...
| FR-18 | Codex managed 每 revision 使用系统临时独立 Git 快照、固定命名 permission profile 和新 thread；profile 必须默认拒绝根目录、只读最小系统路径、仅写精确快照根且关闭网络，不能写业务仓库、读取凭证或继承未隔离的其他 MCP/hooks/plugins |
| FR-19 | 只有显式 required checks 全过、allowed paths/diff/baseline 全部复验后才能自动回写；其他验证结论均 review-required |
| FR-20 | 连接、投递、厂商执行、managed pipeline 四条状态轴独立、单调且有证据；旧 phase 只能由服务端单向派生 |
...
8. Claude attached 和 Codex managed 分别完成“发送 → 执行 → idle → 再发送 → 再执行”的锁定版本真实宿主 E2E；Codex 还必须证明独立快照、验证、回写与污染负向矩阵；未通过时只标记为实验性本地验证。

## 明确范围外

- 远程浏览器、Codespaces、容器和 SSH 主机之间的跨机连接；它们需要独立身份与隧道威胁模型。
- ChatGPT 网页版直接访问本地 stdio 或 loopback。
- 自动选择模型、购买额度、代管外部 Agent 登录或 API Key；SpotPatch 只做脱敏 readiness 诊断。
- 同时编排或广播给多个可写 Agent 竞争修改同一交接。
- 任意 command/args 模板、动态未信任 adapter 插件或从浏览器选择执行权限。
- 通用 Agent 结果回传、聊天消息同步、终端嵌入和会话录制。
- 为 Generic/attached 外部改动自动 Apply/Revert；Codex managed 的窄回写只按 ADR-037 复用既有 Agent 安全原语，不扩展为通用外部 adapter 权限。
