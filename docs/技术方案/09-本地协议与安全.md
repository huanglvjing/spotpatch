...
source-range: "规格书 §2.3、§9.1 末段、§12.3–§12.4、§14–§15、§25.2；v1.1 Agent 本地协议与安全边界；v1.2 多目标授权；v1.3 schema v3 逐目标说明校验；v1.4 编辑器启动确认；v1.5 实际编辑器响应；v1.6 工作区健康与本地修改授权"
参考文献/依赖:
...
| 创建 Job | `POST SPOTPATCH_ENDPOINTS.agentJobs` | `annotation`、`providerProfileId`、`modelProfileId`、值必须为 `true` 的 `providerDataConsent`、`workingTreeMode` | `202`，返回初始 `AgentJobSnapshot` |
...
创建 Job 时客户端不能提供 URL、Key、环境变量名、真实模型名、root、路径、命令、limits、检查集合、system prompt 或 apply mode。服务端必须用 profile ID 从已解析配置重建全部可信值，并重新校验 `SpotAnnotation`、会话、来源和模型 allowlist。`workingTreeMode` 只接受 `require-clean | include-local-changes`，缺失时安全回退为前者；后者只表达本次用户已同意纳入当前本地修改，不授权放宽文件、工具、冲突、规模或 Apply/Revert 门禁。
