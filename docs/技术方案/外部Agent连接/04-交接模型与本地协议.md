...
| External Agent Control contract | Runtime ↔ dev-server 注入端口 ↔ Supervisor | 包内私有 | 脱敏诊断、固定 profile 连接/撤销和四条状态轴；无命令或凭证字段 |
...
`phase` 只允许：`queued | dispatching | dispatched | working | completed | failed | delivery-unknown`。这是 ADR-036 当前 adapter/Registry 的**兼容状态**：其中 `dispatched` 的证据强度由 adapter 决定；Claude Channel 只表示写入 transport，Codex 的 `working/completed` 必须来自匹配 thread/turn 的结构化事件。`completed` 只表示该宿主本轮结束，不表示代码正确、测试通过或变更可撤销。`delivery-unknown` 会把 active adapter 置为 `blocked` 且 `canDispatch: false`，直到用户显式确认已核对工作区。

ADR-037 managed 不能继续把该 `phase` 当产品完成状态。目标 DTO 新增 `connectionState`、`deliveryStatus`、`executionStatus`、`managedPhase` 四条正交轴及单调 `sequence`；定义、证据和迁移规则只在 (见 doc-id:external-agent-10-convergence) 维护。旧 `phase` 在一个发布周期内由服务端从新状态单向派生，Browser 不可提交。
...
- 默认 `false`；手工配置时只有 development phase 且显式 `true` 才装配 Runtime 入口、Store、Broker 和发现描述符。ADR-037 的框架 initializer 会在用户明确运行 `init` 时幂等写入该显式值；这不是运行时环境变量自动开启。
...
| 连接能力/状态 | `/__spotpatch/v1/external-agent/control/status` | 空 JSON | 固定 adapter capability、脱敏诊断、四条状态轴、grant 是否存在和最近稳定错误；无路径/凭证/进程或厂商 ID |
| 请求连接 | `/__spotpatch/v1/external-agent/control/connect` | `{ requestId, adapterKind: "codex", profile: "managed-apply-v1" }` | 幂等返回 `connecting / awaiting-consent / ready / degraded`；不能携带命令、路径、模型或权限参数 |
| 断开/撤销 | `/__spotpatch/v1/external-agent/control/disconnect` | `{ requestId, adapterKind: "codex", revokeGrant: boolean }` | 幂等停止 Supervisor 所有权资源；`revokeGrant` 只删除精确 project key + adapter + profile grant |
| 取消 managed 任务 | `/__spotpatch/v1/external-agent/control/cancel` | `{ requestId, revision }` | 只取消精确当前 managed revision；从未启动/已终态/不匹配均返回稳定冲突结果，不影响 Inbox/attached |
| managed 结果 | `/__spotpatch/v1/external-agent/result` | `{ revision }` | 当前进程内有界结果：allowed paths 的统一 diff、文件/check/timing 摘要和 expiry；无命令原始输出或业务绝对路径 |
| 状态事件 | `/__spotpatch/v1/external-agent/events` | 长连接 POST `{ afterSequence?: number }` | `application/x-ndjson`；首帧完整脱敏状态，此后只发单调 sequence 的有界状态快照与 heartbeat；断线可由 status 补齐 |
...
事件 endpoint 复用 Browser token、Origin、Fetch Metadata、并发/字节/空闲上限和 `AbortSignal`，不使用无法附加自定义认证 header 的原生 `EventSource`。每个 dev Session 最多允许集中上限内订阅；dispose 主动结束 stream。事件只携带与 control status 同一脱敏 DTO，不发送 diff 正文、命令输出、Prompt、thread/turn id 或 token。

managed result 只保留当前 revision，位于 dev-server/runner 进程内存并与 Handoff TTL 同时失效；新 revision 原子替换旧结果。diff 必须在产生时通过 allowed paths、字节和敏感路径审计，响应再次执行固定字节上限。`review-required` 允许用户查看/复制候选 diff，但本专题不提供“忽略 checks 强制 Apply”endpoint；配置好 required checks 后只能创建新 revision。
