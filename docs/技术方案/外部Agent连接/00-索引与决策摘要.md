...
| D14 | Codex managed 由 `pnpm dev` 内唯一 Supervisor 托管，首次 `managed-apply-v1` 在 dev 终端确认 | 页面可配置但不能成为本地写权限授权方；不再要求第二个终端 |
