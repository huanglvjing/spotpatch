...
`playgrounds/minimal-react-18/vite.config.ts` 现固定传入 `dataFlow: {}` 与 `externalAgent: true`，和 `packages/vite/src/initializer.ts` 的能力装配保持一致。内建 AI fake provider 仍只在 `SPOTPATCH_E2E_AI_UI=1` 时启用，不把测试 provider 混入日常开发。
...
| --- | --- | --- | --- | --- |
| Codex | App Server、MCP | managed/attached 均为 `local-validation` | 已安装 `codex-cli 0.150.1` | 版本解析、当前版本 Schema 合同及无 thread 的真实 initialize/account/model/config preflight 通过；真实 managed turn Gate 仍待显式终端授权后执行 |
...
2026-08-27 对官方 npm 包 `@openai/codex@0.149.0` 与本机 `codex-cli 0.150.1` 生成的 experimental v2 Schema 做了结构差异审计：bundle 变化为 1329 行新增、92 行删除，证明跨版本变化真实存在；SpotPatch 当前使用的核心请求字段未出现破坏性变化，新增主要落在浏览器/电脑使用要求、MCP runtime status、实时 thread 和多 Agent 状态枚举。由此不能宣称“所有历史和未来版本天然兼容”，但能够承诺：`>=0.149.0` 的稳定版本只要自身生成 Schema 和运行时 preflight 都满足固定安全子集，就无需 SpotPatch 为每个 Codex patch/minor 单独发布版本；不兼容版本明确降级 Inbox。
...
playground 配置不复制公共默认值；`externalAgent` 与 `dataFlow` 只表达该宿主需要覆盖的产品能力。
...
Codex 0.150.1 已完成版本 Schema 生成、与 0.149.0 的差异审计、必需方法合同预检和现有假宿主协议/权限回归；最低版本保持 `0.149.0`，未来上限由逐版本 Schema 与运行时 preflight 取代。packed-package 合同、临时 Git fixture 连续双 revision、取消/崩溃/清理的真实 0.150.1 Gate 仍须完成，未通过前不能把 `local-validation` 提升为稳定支持。
...
- 已生成并审计 0.149.0/0.150.1 App Server Schema，自动化覆盖兼容的未来稳定版本、低于最低版本、畸形/预发布版本和缺失必需方法；
- 通过 packed fixture、权限负向、连续双 revision 和故障清理；
- 完整真实证据通过后才更新唯一实现状态入口与成熟度。

完成标准：本机 0.150.1 真实 Gate 通过；后续稳定版本在无需修改范围常量的情况下，仍必须依次通过版本 Schema 合同与运行时 preflight。不满足固定安全子集时保持明确“不兼容”并给出 Inbox 降级，不能用跳过校验换取表面兼容。
...
- Node `22.23.2` 下，本机真实 `codex-cli 0.150.1` 的 absolute executable 通过 generated Schema 合同；隔离临时 `CODEX_HOME` 的真实无 thread 连接完成 initialize、authenticated account、默认模型 `gpt-5.6-sol` 与 config requirements preflight，随后立即关闭，Schema 与运行目录均已清理。该证据没有创建 Agent thread、没有发起模型 turn，也不替代用户项目 grant。

本轮未通过或未执行的项目必须保持明确：

- 当前运行环境没有可控制的浏览器实例，未执行 UI 肉眼检查，也没有运行 Playwright external Agent 新场景；
- P2 假宿主、完整 external Agent 浏览器状态矩阵、视觉和无障碍 Gate 尚未实现；
- Codex 0.150.1 已通过版本解析、generated Schema 合同和假宿主回归；尚未在本轮完成需要用户显式终端授权的真实 managed 双 revision，因此状态仍是 `local-validation`；
