...
source-range: "规格书 §1、§1.1–§1.3；v1.1 新增可选 AI Agent 产品边界；v1.2 多目标与双语产品化边界；v1.3 Cursor/VS Code 源码导航与开源入口；v1.4 编辑器工作区归属；Next.js 公共预览与正式支持边界；上下文问答规划边界"
参考文献/依赖:
...
## Astro 源码预览补充（2026-09-05）

平级 `@spotpatch/astro` 仅在 `astro dev` 工作；原生模板不要求 React，React 岛屿使用公共 JSX 编译链路。除定位、DOM/CSS、编辑器和 Prompt 外，现已接入公共 AI 审阅/应用/回滚、dataFlow、只读 contextualAsk、externalAgent 和 Astro 专用 trustedFastMode 检查。服务端执行仍不由浏览器证明，动态 DOM/其他岛框架不保证内部 exact。Node >=22.12；当前范围与验收以 [Astro 功能对齐规范](./Astro适配/02-功能对齐实施方案.md)为准。未发布 npm，不扩大外部 Agent 现有成熟度承诺。
...
SpotPatch 规划增加由已选元素约束的 Contextual Ask：至少选择一个元素后，用户可以显式切换 Ask，提交一个问题，并由 Configured Key 或通过兼容 Gate 的 Managed Codex 返回一个带源码引用的只读答案。Ask 与 Change 是协议和权限层面的两种任务，不通过自然语言自动判断；Ask 不创建 worktree、不写代码、不运行 checks，也不产生 Diff/Apply/Revert。

首阶段只做单次问题/答案和当前结果，不做连续追问、长期历史、无目标全仓聊天、Claude/Cursor 答案回流或跨页面 Ask 恢复。“根据回答创建修改”只进入可编辑 Change 草稿，用户再次明确提交后才取得写权限。完整需求、协议、执行、安全、交互和发布 Gate 见 (见 doc-id:context-qa-00-index)。截至 2026-09-01 该能力仍为设计完成、代码未实现，不能列入当前支持矩阵。
...
`@spotpatch/next@0.1.0` 作为 0.x 公共预览分发，但尚未进入正式支持矩阵。`@spotpatch/vite` 不能作为 Next.js 适配器使用；在阻断式 POC、完整兼容矩阵、生产零残留和 Registry 冷安装验收全部通过前，README、包描述和 UI 必须标注“公共预览”，不得声称“正式支持 Next.js”或“支持生产使用”。
