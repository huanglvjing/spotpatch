...
source-range: "规格书 §2.4 第 3 条、§18、§18.1–§18.3、§22；v1.1 Agent 工作台与变更审阅；v1.2 多目标工作台；v1.3 品牌视觉、双语与逐目标编辑；v1.4 低密度 Agent 布局与入口层级；v1.5 源码快捷入口与开源传播；v1.6 深色原生选择控件"
参考文献/依赖:
...
- Provider/Model 原生选择器必须声明深色 `color-scheme`，并为 option 提供与面板一致的前景/背景色，避免深色工作台弹出高亮白色系统菜单；焦点环、禁用态和可访问名称必须保留。不得为了视觉统一复制一套自制下拉状态机或把 option 替换为不可访问的普通 `div`。
