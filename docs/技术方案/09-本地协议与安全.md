...
source-range: "规格书 §2.3、§9.1 末段、§12.3–§12.4、§14–§15、§25.2；v1.1 Agent 本地协议与安全边界"
参考文献/依赖:
...
- 有请求体的 endpoint 仅接受 `application/json`；Job 事件 endpoint 返回 `application/x-ndjson`。
...
| 读取事件 | `GET getAgentJobEndpoint(id, "events")` | 无 body | 返回当前会话内有序 NDJSON 事件 |
| 读取结果 | `GET getAgentJobEndpoint(id, "result")` | 无 body | 返回公共 Job 快照及可用的 `AgentJobResult` |
