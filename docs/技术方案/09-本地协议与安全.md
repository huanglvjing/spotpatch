...
| 创建 Job | `POST SPOTPATCH_ENDPOINTS.agentJobs` | `annotation`、`providerProfileId`、`modelProfileId`、值必须为 `true` 的 `providerDataConsent`、可选且只能为 `true` 的 `trustedFastModeConsent`、`workingTreeMode` | `202`，返回初始 `AgentJobSnapshot` |
...
创建 Job 时客户端不能提供 URL、Key、环境变量名、真实模型名、root、路径、命令、limits、检查集合、system prompt 或 apply mode。服务端必须用 profile ID 从已解析配置重建全部可信值，并重新校验 `SpotAnnotation`、会话、来源和模型 allowlist。`workingTreeMode` 只接受 `require-clean | include-local-changes`，缺失时安全回退为前者；后者只表达本次用户已同意纳入当前本地修改，不授权放宽文件、工具、冲突或规模门禁。`trustedFastModeConsent: true` 也不能选择 apply mode：它只有在服务端已配置 `trusted-auto` 时才有效，配置与字段不一致必须拒绝创建 Job。
...
`trustedFastModeConsent` 缺失表示没有开启可信快速模式，只允许字面量 `true`，不得接受 `false`、字符串或持久化偏好。在 `trusted-auto` 配置下，Runtime 用一个明确说明后果的会话级复选框同时取得 provider 数据传输、纳入当前有界本地修改和通过 required checks 后直接应用的同意，并为每个 Job 重放该显式字段；刷新页面、开发服务重启或切换 provider 后必须重新取得同意。该字段不放宽项目 root、保护文件、符号链接、文本/规模限制、任意 Shell、失败检查、Git 操作状态或基线哈希冲突。
