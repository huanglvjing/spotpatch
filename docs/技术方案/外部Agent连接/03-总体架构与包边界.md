...
- 通用 MCP 连接器不解析 JSX、不接触 DOM，也不拥有文件写入能力；attached adapter 只驱动受宿主 sandbox 约束的 Agent；managed Codex 通过 `@spotpatch/agent` 的窄 runner 写独立快照，Bridge 自身仍不实现文件修改；
- 浏览器会话和本地连接器使用不同认证域；
...
flowchart TB
  ...
  subgraph Managed[SpotPatch managed 执行域]
    MR[@spotpatch/agent Managed Runner]
    SNAP[系统临时独立 Git 快照]
    MR --> SNAP
  end
...
框架生命周期还注入一个窄 `ExternalAgentControlPort`，由唯一 `ExternalAgentSupervisor` 实现诊断、grant、adapter 启停和当前 dev Session 重绑。dev-server 只调用该端口，不 import 厂商 adapter；Supervisor 不创建第二份 Handoff Store 或 Active Registry。
...
ADR-036 的独立主动 Connector 与 ADR-037 的进程内 Supervisor 都复用同一 discovery/Broker client/Event Pump，但额外：
...
- 只把授权后的 snapshot 引用交给一个已知 adapter；managed adapter 再通过窄端口请求 `@spotpatch/agent` 构建独立快照；
...
Claude Code、Cursor 和 Codex 拥有模型、审批和 sandbox。SpotPatch 不把它们的厂商协议塞进 Handoff；Claude attached Channel 只面向已经运行的会话，Codex managed adapter 管理自己启动的 App Server runtime/thread 并把可写根限制到独立快照，Cursor 在没有已验证入站入口前只消费 Generic MCP Inbox。
...
| `@spotpatch/bridge` | MCP/CLI、发现读取、Broker client、设置生成、Supervisor、共享 Event Pump、Claude/Codex 窄 adapter；调用 managed runner 端口 | 源码规则实现、直接文件修改、Git/diff/apply 实现、任意 Shell/命令模板、第二份 Prompt 模型 |
| `@spotpatch/agent` | 工程契约、独立 Git 快照、路径/change-set/check/prepared apply 与 managed runner | 厂商协议、Handoff Store、Browser API、连接状态 |
...
- `bridge -> runtime / compiler / analyzer`；Bridge 只能依赖 `agent` 的公开 Node-only managed runner 端口；
- `vite <-> next`；
