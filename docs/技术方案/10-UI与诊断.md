...
source-range: "规格书 §2.4 第 3 条、§18、§18.1–§18.3、§22；v1.1 Agent 工作台与变更审阅；v1.2 多目标工作台；v1.3 品牌视觉、双语与逐目标编辑；v1.4 低密度 Agent 布局与入口层级；v1.5 源码快捷入口与开源传播；v1.6 深色原生选择控件；v1.7 工作区健康、显式同意与渐进增强选择器"
参考文献/依赖:
...
- 运行态依次呈现准备隔离环境、分析、工具调用、验证和生成审阅结果。工具活动只展示工具名、相对路径或 check label 和脱敏状态，不滚动暴露整段源码或命令环境。
