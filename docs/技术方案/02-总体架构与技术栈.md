...
source-range: "规格书 §2.1–§2.2、§3–§5（§5.1 除外）、§5.2；v1.1 AI Agent 架构扩展；v1.2 产品化 Runtime 模块边界；v1.3 Runtime/React Adapter 浏览器包拆分；v1.4 CI 启动与 npm 发布基线；v1.4.1 跨平台文本基线；v1.5 编辑器适配边界；v1.6 编辑器工作区路由；v1.7 本地工作区健康与隔离快照边界；Next.js 多适配器目标架构"
参考文献/依赖:
  - "01-product-boundary"
...
当前目录树和包关系仍是已发布的 Vite 实现事实。Next.js 支持不通过复制 Vite 包或另建仓库实现；目标是在同一 Monorepo 内抽取框架无关的编译内核与 Node 开发服务，再让 `@spotpatch/vite` 和计划中的 `@spotpatch/next` 成为平级适配器。目标包图、依赖方向、迁移顺序和禁止依赖只有一个规范位置 (见 doc-id:next-02-package-architecture)。

在公共内核抽取完成前，本文件中的现有包结构仍是实现事实；不得提前创建同名空包、跨包深层导入或复制转换/服务逻辑。任何抽取必须在同一变更中迁移规范事实、测试和两个消费方，保证 Vite 零回归。
