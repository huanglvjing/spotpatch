...
shared  ←  runtime
shared  ←  agent  ←  vite
shared  ←  compiler ← analyzer ← dev-server ← vite
...
组件数据链路增加第三个独立浏览器前置产物 `runtime-data-flow-prelude`，只依赖 `@spotpatch/runtime/data-flow` 与 `@spotpatch/shared/data-flow-runtime` 轻量子入口；不得把 UI、Zod 或 Node analyzer 打入该前置包。`@spotpatch/analyzer` 是 Node-only TypeScript semantic 包，由 dev-server 通过窄接口持有；Runtime 只消费严格 report，不导入 analyzer。当前包边界见 (见 doc-id:data-flow-03-architecture-packages)、(见 doc-id:data-flow-13-beta-implementation)。
