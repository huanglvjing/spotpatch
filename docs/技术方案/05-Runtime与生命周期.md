...
- `ready` 直接允许继续；review/auto 的 `consent-required` 必须展示 staged/unstaged/untracked 数量和独立的本地修改纳入同意，用户勾选前 Run 保持禁用；页面选中 trusted-auto 时使用包含远程传输、本地修改、跳过项目检查与直接应用后果的一次会话级同意，不重复显示第二个复选框。`blocked` 必须展示稳定错误码对应的具体原因，任何模式都不能用同意绕过项目边界、Git 状态或冲突门禁。检查中的瞬时状态不得清空用户刚刚作出的同意，最终变为 `ready` 或 `blocked` 时必须清除不再适用的独立工作区同意。
